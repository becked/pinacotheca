...
- **`extractor.py`**: Four extraction entry points called in sequence by `pinacotheca`:
  - `extract_sprites()` — 2D sprite extraction (the original 4000+ icon set). Groups Sprite objects by `m_Name` and keeps **every** co-named variant instead of the old first-write-wins that silently dropped all but whichever the object table yielded first (213 assets across units/crests/religions/traits/theology/other were being lost). Per name, the largest variant keeps the bare `{name}.png`; smaller variants (a unit's ~58px flag glyph, a crest's ~33px list icon) take a `__ICON` role suffix — `variant_output_names()` is the pure, deterministic naming rule (`{name}.png`, `{name}__ICON.png`, `{name}__ICON_2.png` for the lone 3-variant `crown`). Double underscore is deliberate: real sprites end in `_ICON` but none contain `__`, so `{name}__ICON` can't collide with a genuine name. Variants sort by area (then w/h, then stable `path_id`) so the same input always yields the same primary; writes overwrite unconditionally so a prior wrong-variant `{name}.png` self-heals.
...
- **Memory management**: Extraction groups sprites by name, then decodes/writes per group, running `gc.collect()` every 500 names and explicitly deleting image data to handle the ~4000+ sprites without memory issues.
...
GitHub Pages serves from the `gh-pages` branch. Only sprites are deployed (converted to WebP at deploy time, ~200 MB), not textures (~2GB).
...
**Co-named 2D sprite variants use a `__ICON` suffix (same folder as the primary).** For any 2D sprite name that ships more than one Sprite object — a unit's painted portrait + its small white flag glyph, a crest's full art + its list icon — the full-art variant is `<NAME>.png` and the small variant is `<NAME>__ICON.png` (`__ICON_2`, `__ICON_3`, … past the first). They live **in the same category folder**, not a separate directory: a unit's flag glyph is `extracted/sprites/units/UNIT_<NAME>__ICON.png`, a crest's small icon is `extracted/sprites/crests/CREST_<NAME>__ICON.png`. per-ankh reads the glyph for its rail unit markers from `units/UNIT_<NAME>__ICON.png` and falls back to the `<NAME>.png` portrait for the handful of units that ship no glyph. (This supersedes the never-shipped `units_icons/` directory proposed in per-ankh #136 — the glyphs are role-suffixed siblings, not a mirror folder.)
...
The deploy command stages `extracted/` to a temp dir via `rsync -aL --copy-unsafe-links --exclude=sprites/<glob>`, then converts every staged `sprites/**/*.png` to WebP q90 (`cwebp -q 90 -alpha_q 100`, parallel across cores) and deletes the source PNGs. WebP q90 is visually lossless on these renders (PSNR 40–44 dB, SSIM > 0.999) at ~18% of the PNG size — benchmarked across improvements, units, portraits, backgrounds, and 2D icons; the full deploy drops from ~957 MB to ~204 MB (~20% of cap). `extracted/` itself is never modified — staging is a deploy-time view, and the local tree stays PNG for per-ankh and the dev server. `rsync` and `cwebp` are both **required** (not soft deps): conversion is load-bearing because the deployed gallery references `.webp` (see below), so a missing `cwebp` aborts the deploy rather than shipping PNGs that won't match the manifest.

**WebP only exists in the deployed gallery.** The SvelteKit gallery picks the image extension at runtime from `$app/environment`'s `dev` flag via `web/src/lib/utils/spriteUrl.ts` (`spriteSrc`/`SPRITE_EXT`): `.png` in the dev server (`pinacotheca-web`, serves the local PNG tree), `.webp` in the production build. `manifest.json` still stores `.png` paths; the swap is centralized in that one helper, used by every `<img>` consumer (SpriteCard, CategoryCard, ModCard, the lightbox in `+page.svelte`) and the download button. **Known gotcha:** opening the *built* `extracted/index.html` directly shows broken images (bundle references `.webp`, local tree is still `.png`); preview with the dev server instead. The deployed site is correct because `pinacotheca-deploy` converts the staged copy.

To change the filter: edit `GALLERY_EXCLUDE_GLOBS` and re-run `pinacotheca`. To inspect filtered files: browse `extracted/sprites/` directly. CLI flags: `--no-filter` is an emergency override that deploys everything (warns it'll exceed the cap), `--dry-run` stages + converts + reports without pushing.
