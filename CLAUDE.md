...
**Capital + urban PNGs embed ground.** As of the layered-render work, `IMPROVEMENT_3D_<NATION>_CAPITAL.png` (12 nations) and `IMPROVEMENT_3D_<NATION>_URBAN.png` (10 nations) include a `TERRAIN_TEMPERATE` biome hex tile + per-nation `TerrainTexturePVTSplat` paint underneath the buildings. Per-ankh consumers must NOT double-render terrain underneath these — the hex ground is part of the icon. All other `IMPROVEMENT_3D_*.png` outputs (regular improvements, supplemental prefabs) and `RESOURCE_3D_*.png` outputs are unchanged: still transparent backgrounds. See `docs/extracting-3d-buildings.md` (Ground layer section) and `src/pinacotheca/layered_render.py` for the design.
