...
Currently excluded: per-(improvement, nation) urban composites (`improvements/IMPROVEMENT_3D_*_*_URBAN.png`, ~1.3 GB / 719 files). **per-ankh continues to read `extracted/sprites/` directly and is unaffected by the filter** — this is the load-bearing invariant of the design. Pattern contract: only `*` wildcards (no `?`, `[...]`, `**`); `*` does not cross `/`. Validated at module load; parity with the TS-side regex translation is verified by `tests/test_gallery_filter.py::TestPythonTSParity`.
